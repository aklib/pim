...
var addEvent = U.addEvent, createElement = U.createElement, css = U.css, defined = U.defined, discardElement = U.discardElement, erase = U.erase, extend = U.extend,
    extendClass = U.extendClass, getOptions = U.getOptions, isArray = U.isArray, isNumber = U.isNumber, isObject = U.isObject, merge = U.merge, offset = U.offset, pick = U.pick,
    pInt = U.pInt, uniqueKey = U.uniqueKey;
...
if (!svg) {
    ...
    VMLElement = {
        ...
        getSpanCorrection: function (width, baseline, alignCorrection, rotation, align) {
            var costheta = rotation ? Math.cos(rotation * deg2rad) : 1, sintheta = rotation ? Math.sin(rotation * deg2rad) : 0,
        ...
        shadow: function (shadowOptions, group, cutOff) {
            var shadows = [], i, element = this.element, renderer = this.renderer, shadow, elemStyle = element.style, markup, path = element.path, strokeWidth, modifiedPath,
    ...
    VMLRendererExtension = {
        ...
        clipRect: function (x, y, width, height) {
            ...
            return extend(clipRect, {
                ...
                getCSS: function (wrapper) {
                    var element = wrapper.element, nodeName = element.nodeName, isShape = nodeName === 'shape', inverted = wrapper.inverted, rect = this,
                        top = rect.top - (isShape ? element.offsetTop : 0), left = rect.left, right = left + rect.width, bottom = top + rect.height, ret = {
                            clip: 'rect(' +
        ...
        color: function (colorOption, elem, prop, wrapper) {
            ...
            if (fillType) {
                var stopColor, stopOpacity, gradient = (colorOption.linearGradient ||
                    colorOption.radialGradient), x1, y1, x2, y2, opacity1, opacity2, color1, color2, fillAttr = '', stops = colorOption.stops, firstStop, lastStop, colors = [],
                ...
                if (prop === 'fill') {
                    ...
                    if (fillType === 'gradient') {
                        ...
                        var r = gradient.r, sizex = r * 2, sizey = r * 2, cx = gradient.cx, cy = gradient.cy, radialReference = elem.radialReference, bBox,
        ...
        symbols: {
            ...
            arc: function (x, y, w, h, options) {
                var start = options.start, end = options.end, radius = options.r || w || h, innerRadius = options.innerR, cosStart = Math.cos(start), sinStart = Math.sin(start),
